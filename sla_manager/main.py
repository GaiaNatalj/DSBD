...
def enterMetricCodes(number_metric):
    ...
    #print(code_list)
    while len(metrics_sla) < number_metric:  # mettere 5
        code_metric = input('\nInsert metric: ')
...
def pastViolations(metrics_sla, allowed_values):
    for key in range(0, len(metrics_sla)):
        ...
        for h in range(0, len(start_time)):
            ...
            #past_violation_metric["time_"+str(start_time[h])]={"rangeMin": rangeMin, "rangeMax": rangeMax,"violations":num_violazioni}
            if violazione == 1:
...
def kakfaResultProducer(fileJson,keyword):
    ...
    #print("Configurazione del profucer: ", conf)
